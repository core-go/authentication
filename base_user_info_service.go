...
func (s *BaseUserInfoService) Pass(ctx context.Context, user UserInfo) error {
	...
	if user.Deactivated == true {
		_, er1 := s.AuthenticationRepository.PassAndActivate(ctx, user.Id)
		return er1
	}
	_, er2 := s.AuthenticationRepository.Pass(ctx, user.Id)
	return er2
}
...
func (s *BaseUserInfoService) Fail(ctx context.Context, user UserInfo) error {
	...
	if s.LockedMinutes > 0 && s.MaxPasswordFailed > 0 && user.FailCount >= s.MaxPasswordFailed {
		...
		return s.AuthenticationRepository.Fail(ctx, user.Id, 0, &lockedUntilTime)
	}
	count := user.FailCount + 1
	return s.AuthenticationRepository.Fail(ctx, user.Id, count, nil)
}
