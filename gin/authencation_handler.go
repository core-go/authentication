...
func (h *AuthenticationHandler) Authenticate(ctx2 *gin.Context) {
	r := ctx2.Request
	var user a.AuthInfo
	if strings.Contains(ctx2.Request.Header.Get("Content-Type"), "multipart/form-data") {
		if err := ctx2.Request.ParseMultipartForm(1073741824); err != nil {
			ctx2.String(http.StatusBadRequest, "cannot parse form: "+err.Error())
			return
		}
		...
		if err != nil {
			...
			ctx2.String(http.StatusBadRequest, "cannot decode authentication info")
			return
		}
		...
		for k, v := range postForm {
			if index, ok := mapIndexModels[k]; ok {
				...
				if strings.Index(idType, "int") >= 0 {
					...
					if err != nil {
						ctx2.String(http.StatusBadRequest, "invalid key: "+k)
						return
					}
	...
	if h.Decrypt != nil {
		if decodedPassword, er2 := h.Decrypt(user.Password); er2 != nil {
			...
			ctx2.String(http.StatusBadRequest, "cannot decrypt password")
			return
		} else {
	...
	if er3 != nil {
		...
		if result.Status == h.Timeout {
			respond(ctx2, http.StatusGatewayTimeout, "timeout", h.Log, h.Resource, h.Action, false, er3.Error())
		} else {
			result.Status = h.SystemError
			respond(ctx2, http.StatusInternalServerError, result, h.Log, h.Resource, h.Action, false, er3.Error())
		}
	} else {
		if h.Whitelist != nil {
		...
		if h.Cookie {
			...
			if strings.Contains(host, h.Host) || strings.Contains(host, "localhost") {
				...
				if err != nil {
					respond(ctx2, http.StatusInternalServerError, nil, h.Log, h.Resource, h.Action, false, err.Error())
					return
				}
			...
			if token == "" {
				ctx2.String(http.StatusUnauthorized, "cannot get token")
				return
			}
			...
			if h.Store != nil && h.Generate != nil && len(h.Host) > 0 {
				if h.SingleSession {
					...
					if len(data1) > 0 {
						...
						if len(sid) > 0 {
							...
							if err2 != nil {
								respond(ctx2, http.StatusInternalServerError, nil, h.Log, h.Resource, h.Action, false, err2.Error())
								return
							}
						}
					}
				...
				http.SetCookie(ctx2.Writer, &http.Cookie{
					Name:     h.CookieName,
					Domain:   host,
					Value:    sessionId,
					HttpOnly: true,
		...
		respond(ctx2, http.StatusOK, result, h.Log, h.Resource, h.Action, true, "")
	}
}
func (h *AuthenticationHandler) Logout(ctx2 *gin.Context) {
	cookie, err := ctx2.Request.Cookie(h.CookieName)
	if err != nil {
		respond(ctx2, http.StatusInternalServerError, "", h.Log, h.Resource, h.Action, false, err.Error())
		return
	}
	if cookie == nil || cookie.Value == "" {
		respond(ctx2, http.StatusOK, expired, h.Log, h.Resource, h.LogoutAction, true, "")
		return
	}
	...
	if h.DecodeSessionID != nil {
		...
		if err != nil {
			respond(ctx2, http.StatusInternalServerError, "", h.Log, h.Resource, h.LogoutAction, false, err.Error())
			return
		}
	}
	...
	if err != nil {
		if err.Error() == "redis: nil" {
			respond(ctx2, http.StatusOK, 1, h.Log, h.Resource, h.LogoutAction, true, err.Error())
			return
		}
	}
	...
	respond(ctx2, http.StatusOK, 1, h.Log, h.Resource, h.LogoutAction, true, "")
}
